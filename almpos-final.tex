...
\subsection{}
In this appendix we prove a topological theorem.
It implies  the main conjecture \ref{con:tor} in the case if \emph{all the Alexandrov spaces $A_i$ which appear in the construction in  Section \ref{sec:constr} for all possible choices of base points are closed Riemannian manifolds}.

The proof of \ref{thm:top} which we present is surprisingly involved.
...
The assumption in the theorem  capture the topology of the main source of examples of the needed collapse, so it worth to consider even if you do not believe in the reduction of the special case of the main conjecture to the theorem \ref{thm:top} 
...
The reduction of the described case of the main conjecture to the theorem \ref{thm:top} mainly follows from Yamaguchi's fibration theorem but the proof requires some work:
(1) The fibration theorem gives a tower of bundles build from above meaning that on each step, we use already constructed tower as a fiber.
We need to reconstruct this tower from below meaning that we use the constructed tower as the base of next bundle.
The spaces $A_i$ become the fibers of obtained tower; by passing to a subsequence, we may assume that the topology of these spaces is fixed.
(2) We need to pass to finite covering (with number of folds bounded in terms of $A_i$) so that each bundle admits a trivialization over a 1-dimensional skeleton of the base and each fiber bundle splits into bundles with circles and simply connected spaces as fibers --- to prove it we use an argument similar to the one given in Section~\ref{sec:nilp}.
We omit the proof of this implication since the assumption on $A_i$ is unnatural  
which gives no geometric value to the conclusion.
