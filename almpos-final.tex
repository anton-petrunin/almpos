...
\quad\text{and}\quad 2{\cdot} \diam(M_n)\le r_2-r_1.$$
...
\begin{thm}
Let $F_1,F_2,...,F_n$ be an array of compct manifolds 
...
Assume
$\kappa_z\colon F\to F$ is a one parameter of homotopy equivalences, $z\in\mathbb{S}^1$,
...
We'll denote the resulting map $H^{*+2}(F)\to H^*(F)$ by $D_h$. It's easy to see that it's an algebra derivation on $H^*(F)$. The same obviously holds true for $D_h^\mathbb{Q}\colon H^{*+2}(F,\mathbb{Q})\to H^*(F,\mathbb{Q})$

%Then it the correspondent derivation 
%$$D_h\colon H^{k+2}(F)\to H^{k}(F)$$
 %has nontrivial element in the second cohomology; that is,
 We claim that  $D_h^\mathbb{Q}$ vanishes on $H^2(F,\mathbb{Q})$. Suppose not and 
  for some 
$\theta\in H^2(F,\mathbb{Q})$ we have $H^0(F,\mathbb{Q})\ni D_h^\mathbb{Q}\theta\not=0$. 
Note that \[D_h^\mathbb{Q}\theta^k=k{\cdot} \theta^{k-1}{\cdot} D_h^\mathbb{Q}\theta.\] 
...
Thus, $D^\mathbb{Q}_h|_{H^2(F,\mathbb{Q})}\equiv 0$. Hence, $D_h|_{H^2(F)}\equiv 0$ and therefore $h^*=pr_2^*$ on $H^2(F)$ where $pr_2\colon \mathbb{S}^2\times F\to F$ is the second coordinate projection. 
...
which admits a trivialization on $1$-skeleton of $B$. 
...
\begin{claim}
 Let $\mathbb{S}^1\to E\to B$ be an oriented $\mathbb{S}^1$-bundle, the base  $B$ is reliable and the action of $\pi_1B$ on $\pi_2B$ is almost trivial. 
Then $E$ is reliable.
\end{claim}
...
By the definition of a reliable manifold there is a covering $\iota\colon \tilde T^2\z\to \mathbb{S}^1\times \mathbb{S}^1$ and a map of solid torus $\bar f\colon \DD\times \mathbb{S}^1\z\to B$ and an isomorphism $i\colon \tilde T^2\to \partial \DD\times \mathbb{S}^1$ such that $\bar f\circ i=\bar h\circ\iota$. The pullback bundle $\mathbb{S}^1\to \tilde E'\to \DD\times \mathbb{S}^1$ is trivial. Let $s\colon  \DD\times \mathbb{S}^1\to  \tilde E'$ be a section.
...
\bar h(u,*)\sim\bar\gamma\ \ \text{ and}\ \ 
\bar h(*,v)\sim\bar\tau,$$
which satisfy the condition in the  definition of reliable manifold and such that the induced bundle $\bar h^*$ over $\mathbb{S}^1\times \mathbb{S}^1$ and fiber $F$ is homotopically trivial.
...
After finite cover $\iota\colon \tilde T^2\to \mathbb{S}^1\times \mathbb{S}^1$ the induced bundle 
...
\begin{enumerate}
\item The  induced bundle $f^*$ over $\DD\times \mathbb{S}^1$ is trivial since the bundle over $\mathbb{S}^1$ is. 
\item There is yet another homotopic trivialization over $\partial \DD\times \mathbb{S}^1=\widetilde T^2$ which is induced from a homotopic trivialization on $T^2=\mathbb{S}^1\times \mathbb{S}^1$.
\end{enumerate}



Let us lift the homotopy $\DD\times \mathbb{S}^1\to B$ to a homotopy 
$m\colon \DD\times F\times \mathbb{R}\to E$ 
(here $\mathbb{S}^1=\mathbb{R}/\mathbb{Z}$) on such a way that on its boundary $\partial \DD\times F$ it goes along the second trivialization over $\partial \DD\times \mathbb{S}^1=\widetilde T^2$.
For all $u\in \DD$ and  $x\in F$, define $\hat h\colon\DD\times F\to F$ as the (nececary unique) map such that 
\[m(u,x,0)=m(u,\hat h(x),1).\] 
Note that $\hat h(u,x)=x$ for any $u\in\partial \DD$ and $x\in F$;
it means that we can apply Technical Lemma (\ref{techlem}) for $\hat h$.

Fix $x\in F$, $w\in \mathbb{S}^1$.
Since $F$ is simply connected, we can choose a conitnuous map $f\:\colon \DD\to F$ such that $f(u)=x$ for any $u\in \partial \DD$.
...
the map of solid torus is a concatination of $(u,t)\mapsto m(u,f(u),t)$ for $t\in[0,n]$ 
