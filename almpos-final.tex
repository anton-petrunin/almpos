...
The implication mainly follows from Yamaguchi's fibration theorem, but the proof requires some work:
(1) the fibration theorem gives a tower of bundles build from above meaning that on each step, we use already constructed tower of bundles as a fiber
and we need to reconstruct this tower from below meaning that we use the constructed tower as the base of next bundle --- $A_i$ become the fibers of obtained tower; 
(2) we need to pass to controlled covering so that each fibers split into bundles admit trivialization over 1-dimensional skeletons and each fiber bundle splits into bundles with circles and simply connected spaces as fibers --- to prove it we use an argument similar to the one given in Section~\ref{sec:nilp}.
...
The assumption in the theorem seem to capture the topology of the main source of examples of the needed collapse, so it worth to consider even if you do not believe in the reduction of the special case of the main conjecture to the theorem \ref{thm:top} 
