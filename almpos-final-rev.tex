...
%using 1-strainers coming from ${\Bbb S}^{1}$
...
%\lbl[l]{25,160;$\times \lambda_n\approx\ \  ??$}
%\lbl[l]{20,91;$M_n$}
%\lbl[b]{12,21;$q_n$}
%\lbl[b]{12,20;\Large{.}}
%\lbl[l]{25,22;$\times \lambda_n\approx {\Bbb S}^2\times {\Bbb S}^{1}\times {\Bbb R}$}
%\lbl[br]{0,55;$p$}
%\end{lpic}
%\end{floatingfigure}
...
\begin{thm}
Let $F_1,F_2,...,F_n$ be an array of compact manifolds 
...
Assume
$\{\kappa_z\colon F\to F: z\in \S^1\}$ is a one parameter of homotopy equivalences with $\kappa_1=\id_F$
...
\begin{claim}
Let ${\Bbb S}^1\to E\to B$ be an oriented ${\Bbb S}^1$-bundle over a compat manifold $B$.
...
\begin{claim}
 Let ${\Bbb S}^1\to E\to B$ be an oriented ${\Bbb S}^1$-bundle such that the base  $B$ is reliable and the action of $\pi_1B$ on $\pi_2B$ is almost trivial. 
Then $E$ is reliable.
\end{claim}
...
\bar h(u,*)\sim\bar\gamma\ \ \text{ and}\ \ 
\bar h(*,v)\sim\bar\tau,$$
which satisfy the condition in the  definition of reliable manifold and such that the induced bundle $\bar h^*$ over ${\Bbb S}^1\times {\Bbb S}^1$ with  fiber $F$ is homotopically trivial.
...
Pulling back by the finite cover $\iota\colon \tilde T^2\to {\Bbb S}^1\times {\Bbb S}^1$ the induced bundle 
...
\begin{enumerate}
\item \label{triv1} The  induced bundle $f^*$ over $\DD\times {\Bbb S}^1$ is trivial since the bundle over ${\Bbb S}^1$ is. 
\item \label{triv2} There is yet another  trivialization over $\partial \DD\times {\Bbb S}^1=\widetilde T^2$ which is induced from a  trivialization on $T^2={\Bbb S}^1\times {\Bbb S}^1$.
\end{enumerate}


We can assume  that the bundle over $\DD\times \S^1$ is smooth and both of the above trivializations are smooth. Consider the vector field $\bar V=\frac {\partial }{\partial t}$ on $\DD\times \S^1$. (here we identify  $\S^1$ with $\R/\Z$).

Let $p\co E'\to \DD\times \S^1$ be the pullback bundle over  $\DD\times \S^1$. 

Let $ V$ be the horizontal lift of $\bar V$ over $\tilde T^2$ with respect to  trivialization \eqref{triv2} above. Using partition of unity we can easily extend it to a vector field (which we'll still denote by $V$) on $E'$  which projects to $\bar V$. Let $F\co E'\to E'$ be the time 1 integral flow of $V$. Note that by construction 
\begin{equation}\label{monodr-bry}
F(q)=q \text{ for any } q \in p^{-1}(\partial \DD\times\S^1).
\end{equation}

Since $E'$ is a trivial bundle we can identify its total space with $\DD\times F\times \S^1$ via, say, trivialization~\eqref{triv1} above. 


Consider the map  $m\co \DD\times F\times {\Bbb R}\to E$ given by the formula $(u,x,t)\mapsto \phi_t((u,x,1))$ where $\phi_t$ is the integral flow of $V$ where $u\in \DD, x\in F, 1\in \S^1$.

Observe that  by \eqref{monodr-bry} it follows that for any $u\in \partial \DD, x\in F$ we have that $m(u,x,n)=m(u,x,0)$ for any $n\in\Z$.

Define $\hat h\co \DD\times F\to \DD$ by the formula $\hat{h}(u,x)=pr_F\circ m(u,x,1)$ where $pr_F\co \DD\times F\times \S^1\to F$ is the coordinate projection.


Note that by above, $\hat h(u,x)=x$ for any $u\in\partial \DD$ and $x\in F$.
This means that we can apply Technical Lemma (\ref{techlem}) to $\hat h$.

Consider a horizontal section of $E'$ over $\partial \DD\times \S^1$ with respect to ~\eqref{triv2}. Fix $w\in \mathbb{S}^1$. This gives a horizontal section over $\partial \DD\times w$.

Since $F$ is simply connected this section can be extended to a section over $\DD\times w$.




Fix $x\in F$, $w\in {\Bbb S}^1$.
Since $F$ is simply connected, we can choose a conitnuous map $f\:\colon \DD\to F$ such that $f(u)=x$ for any $u\in \partial \DD$.{\color{red} What???}
Recall that $f_{\hat h}(u)= \hat h(u,f(u))$
and by Technical Lemma (\ref{techlem})
\begin{equation}
\label{eq:in-Tor}
[f_{\hat h}]-[f]=[\beta_x]\in \mathrm{Tor}(\pi_2F)
\end{equation}


Consider the sequence of maps $f^n\colon \DD\to F$ defined recurcevly, $f^0=f$ and $f^{n+1}=f^n_{\hat h}$.
By (\ref{eq:in-Tor}),
\begin{equation}
\label{eq:in-Tor-n}
[f^n]-[f^0]=n\cdot\beta_x=0
\end{equation}
