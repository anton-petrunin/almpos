...
A source of examples of almost nonnegatively curved manifold is provided by the total spaces of towers of fiber bundles of certain type. We prove the conclusion of the torsion in center conjecture for these examples.
\end{abstract}


\section{Introduction}
A basic result in the study of manifolds with sectional curvature bounded below is the following fibration theorem of Yamaguchi~\cite{Yam}:
\begin{thm}[Yamaguchi's Fibration Theorem]
Let $M_n$ be a sequence of $m$-dimensional Riemannian manifolds with $\sec\ge k,\diam\le D$, Gromov-Hausdorff converging to a Riemannian manifold $N$. Then for all large $i$ there exists an almost Riemannian submersion $f_i\co M_i\to N$. In particular, $f_i$ is a fiber bundle.
\end{thm}

The fibers of the fibrations that occur in Yamaguchi's fibration theorem are essentially \emph {almost nonnegatively curved}.
\begin{defn}
...
\end{mconj}
Let us also recall the following earlier conjecture Fukaya and Yamaguchi \cite{FY}.
%This conjecture generalizes the following conjecture formulated by Fukaya and Yamaguchi in \cite{FY}.
...
Since the fundamental group of a positively curved manifold is finite, Conjecture ~\ref{con:tor} immediately implies that the fundamental
group of a closed positively curved $m$-manifold is $C(m)$-abelian.

The following argument (cf.  \cite[Corollary 4.6.1]{KPT}) suggested to us by Burkhard Wilking shows that Conjecture~\ref{con:tor} implies Conjecture~\ref{con:c-ab} in general.

Indeed, if $\sec(M^m)\ge0$, then the universal cover $\tilde M$ of $M$ is isometric to the product $\mathbb{R}^n\times K$, where $K$ is a compact Riemannian manifold and the $\pi_1M$ action  on $\mathbb{R}^n\times K$ is diagonal~\cite{CG72}.
...
A natural approach to studying almost nonnegatively curved manifolds (and, more generally, collapsing with lower sectional curvature bound) is by studying successive blow-ups of the
collapsing sequence $M_n$ as done in \cite[Section 4.3]{KPT}:

Let $M_n^m$ be a sequence of closed manifolds satisfying 
$$\sec(M,g_n)\ge -1/n\quad\text{and}\quad\diam(M,g_n)\le 1/n.$$


It clearly Gromov-Hausdorff converges to a point $=:A_0$.
...
the Alexandrov spaces $A_1,\dots, A_\ell$ are closed  Riemannian manifolds (note that this assumption definitely \emph{need not} hold in general).

\begin{conj}[Generalized Yamaguchi's fibration theorem]
After passing to a subsequence of $M_n$,
...
Moreover, by using the Lipschitz properties of the gradient push it should be possible to further refine this tower of bundles to a tower of the form
...
Let us note that our proof of Theorem~\ref{thm:smooth} is surprisingly complicated. We would be quite interested in finding an easier proof among other reasons because it might be easier to generalize to proving Conjecture~\ref{con:tor} in general.

\section{Proof of Theorem \ref{thm:smooth}}
In the proof of \ref{thm:smooth} we use the following result proved by Dror,  Dwyer and Kan in \cite{DDK}.
