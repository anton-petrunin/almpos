...
(An appendix for ``Nilpotency, almost nonnegative\\
...
The motivation for the following topological result comes from geometry;
it is decried in the next section.
...
Our proof of Theorem~\ref{thm:smooth} is surprisingly complicated.
We would be quite interested in finding an easier proof among other reasons because it might be easier to generalize to proving Conjecture~\ref{con:tor} below.
...
Let us show that our main conjecture (if true) implies the conjecture above.

If the fundamental group of $M$ is finite (in particular if $M$ is positively curved) then whole group is torsion.
Therefore Conjecture~\ref{con:tor} implies Conjecture~\ref{con:c-ab}.
...
\subsection{} We will explain now how Theorem \ref{thm:smooth} is related to the main conjecture.
...
A natural approach to studying almost nonnegatively curved manifolds is by studying successive blow-ups of the collapsing sequence $M_n=(M,g_n)$ as done in \cite[Section 4.3]{KPT}:

The sequence $(M_n)$ clearly Gromov--Hausdorff converges to a point in the sense of Gromov--Hausdorff.
This one-point space will be denoted by $A_0$.
...
Let us call it \emph{rescaled smoothness assumption}.
This assumption definitely \emph{need not} hold in general;
it is made only to simplify the problem.
Making this assumption makes possible to apply the following fibration theorem of Yamaguchi~\cite{Yam}:
...
The following statement we formulate as a conjecture, 
but likely we have a proof of it.
However the rescaled smoothness assumption???

\begin{conj}[Generalized Yamaguchi's fibration theorem]
Assume $M_n$ as above satisfies the rescaled smoothness assumption.
Then after passing to a subsequence of $M_n$ and controlled covers,
...
The fibers of the fibrations that occur in Yamaguchi's fibration theorem are \emph {almost nonnegatively curved} in a generalized sense.
