...
every almost nonnegatively curved closed smooth $m$-dimensional manifold $M$ 
...
Our proof is  actually surprisingly involved and 
we would be very much interested in any simpler one which,
in particular, might help to establish the conjecture in  full generality.
...
The general case is more tricky, and
the following argument (cf. \cite[Corollary 4.6.1]{KPT}) to prove it 
was suggested to us by Burkhard Wilking.
...
\subsection{} Let us next explain  how Theorem~\ref{thm:smooth} is related to Conjecture~\ref{con:tor}.

An almost nonnegatively curved manifold can be defined as a closed smooth manifold $M$ which admits a sequence of metrics $g_n$ with a uniform lower curvature bound w.r.t. which it converges to a point in Gromov--Hausdorff topology.
...
ILet $M_n$ as above satisfies the rescaled smoothness assumption.
